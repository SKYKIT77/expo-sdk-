...
import { db, safeFirestoreCall } from '../../firebase';
import { 
  collection, 
  query, 
  getDocs, 
  ...
  useEffect(() => {
    ...
    const setupSubscriptions = async () => {
      try {
        ...
        unsubscribeSchedules = await safeFirestoreCall(() => 
          onSnapshot(schedulesQuery, {
            next: (snapshot) => {
              const scheduleList = [];
            ...
            error: (error) => {
              ...
              Alert.alert('Error', 'ไม่สามารถดึงข้อมูลตารางฝึกซ้อมได้ กรุณาลองใหม่อีกครั้ง');
              setLoading(false);
            }
          })
        );
        ...
        unsubscribePlayers = await safeFirestoreCall(() => 
          onSnapshot(playersQuery, {
            next: (snapshot) => {
              ...
              snapshot.forEach((doc) => {
                ...
                if (userData.role !== 'admin') {
                  playersList.push({ id: doc.id, ...userData });
                }
              });
            ...
            error: (error) => {
              ...
              Alert.alert('Error', 'ไม่สามารถดึงข้อมูลผู้เล่นได้ กรุณาลองใหม่อีกครั้ง');
            }
          })
        );
      } catch (error) {
        console.error("Setup subscriptions error:", error);
        Alert.alert('Error', 'เกิดข้อผิดพลาดในการเชื่อมต่อ กรุณาลองใหม่อีกครั้ง');
        setLoading(false);
      }
    };
  ...
  const handleAddSchedule = async () => {
    ...
    try {
      const scheduleData = {
        ...
        status: 'upcoming',
        totalDuration: parseInt(newSchedule.warmup.duration) +
                      parseInt(newSchedule.mainPhase.duration) +
                      parseInt(newSchedule.cooldown.duration)
      };

      const docRef = await addDoc(collection(db, 'trainings'), scheduleData);
      
      // เพิ่มการอัพเดท real-time
    ...
    Alert.alert(
      ...
      [
        { text: 'ยกเลิก', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await deleteDoc(doc(db, 'trainings', scheduleId));
              // Notifications will be updated automatically due to real-time listener
